...
void SResponseCurveWidget::Construct(const FArguments& InArgs) {}

FVector2D SResponseCurveWidget::GetWidgetPosition(float X, float Y, const FGeometry& Geom) const
{
	return FVector2D((X * Geom.GetLocalSize().X), (Geom.GetLocalSize().Y - 1) - (Y * Geom.GetLocalSize().Y));
}

int32 SResponseCurveWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect,
...
FVector2D SResponseCurveWidget::ComputeDesiredSize(float) const
{
